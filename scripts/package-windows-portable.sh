...
# productName is QNwork; Cargo package name produces qnwork.exe.
find_release_exe() {
  local name
  for name in QNwork.exe qnwork.exe Grok.exe grok-app.exe; do
...
STAGE="dist-portable/QNwork_${VER}_x64-portable"
rm -rf dist-portable
mkdir -p "$STAGE"
# Always ship as QNwork.exe for end users (product name).
cp "$EXE" "$STAGE/QNwork.exe"
...
(stage / "README-portable.txt").write_text(
    f"""QNwork portable (绿色版) v{ver}
================================
1. 解压本目录到任意位置（无需安装）。
2. 双击 QNwork.exe 运行。
3. 需要系统已安装 Microsoft Edge WebView2 Runtime（Win10/11 通常已自带）。
4. Agent 能力仍需本机 Grok Build CLI（grok.exe）并完成登录。
5. SmartScreen 可能提示未知发布者 → 更多信息 → 仍要运行。

Extract anywhere and run QNwork.exe. WebView2 required. Grok Build CLI still needed for agent sessions.
""",
    encoding="utf-8",
)
...
OUT="QNwork_${VER}_x64-portable.zip"
# Prefer zip; on Windows Git Bash it is usually present. Fallback to PowerShell Compress-Archive.
if command -v zip >/dev/null 2>&1; then
  (cd dist-portable && zip -r "../${OUT}" "QNwork_${VER}_x64-portable")
else
  powershell.exe -NoProfile -Command \
    "Compress-Archive -Path 'dist-portable/QNwork_${VER}_x64-portable' -DestinationPath '${OUT}' -Force"
fi
ls -lah "$OUT"
