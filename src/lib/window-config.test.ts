...
describe("window chrome", () => {
  ...
  it("ships Windows shell integration for Show Desktop (frameless alone)", () => {
    ...
    expect(conf.identifier).toBe("com.qnwork.desktop");
  ...
  it("base product identity is QNwork", () => {
    const conf = JSON.parse(readFileSync(CONF_PATH, "utf8")) as {
      productName?: string;
      app: { windows: Array<{ title?: string }> };
    };
    expect(conf.productName).toBe("QNwork");
    expect(conf.app.windows[0]!.title).toBe("QNwork");
  });
